...
export default function PaymentConfirmationPage(){
    ...
    type ListData = { pricing: {subtotal: number; delivery_fee: number}; order_id: string; cart_items: ItemsArray; };
    ...
    return (
        <View className=' bg-white w-full h-full flex items-center'>
            ...
            <ScrollView>
            <View className='bg-white w-full mb-5 relative flex flex-row items-start justify-center h-80'>
            ...
            <View className='flex items-center px-5 space-y-2 w-full mt-8'>
            <TouchableOpacity
                ...
                <TouchableOpacity
                onPress={()=>{router.replace('/payment')}}
                className={`text-center bg-gray-100 ${(cartItems.length === 0 || loading || loadSignal) && 'bg-gray-500'} relative rounded-xl w-[80%]  p-4 self-center flex items-center justify-around`}
                >
                    {(loadSignal) && (
                        <View className='absolute w-full top-4'>
                            <ActivityIndicator size="small" color="#000000" />
