...
export default function CreateProduct(){
    ...
    const validateInput = () =>{
      if(id){
        if((mealName !== mealData?.meal_name) || (parseInt(mealPrice) !== parseInt(mealData?.price)) || (parseInt(mealCategory) !== mealData?.category[0]) || (description !== mealData?.meal_description) || ((priceDiscount + "") !== (mealData?.discount + "")) || (thumbnail !== mealData?.thumbnail)){
          // alert(`${mealCategory}-${mealData?.category[0]}`)
          return true;
        }
      }else{
      ...
      const fetchCategories = async () => {
          try {
              ...
              if(id){
                ...
                setMealCategory(mealDetails.category[0] + '')
                setThumbnail(mealDetails.thumbnail)
      ...
      try {
        if(!loading && validateInput()){
          ...
          if(id){
            ...
            if(thumbnail != mealData?.thumbnail){
              const file = {
                uri: thumbnail,
                name: 'upload.jpg',
                type: 'image/jpeg',
            ...
            const updatedMeal = await patchRequest<mealInfo>(`${ENDPOINTS['inventory']['meal']}${id}`, formData, true, true,);
            // alert(updatedMeal)
          }else{
          ...
          Toast.show({
            type: 'success',
            text1: id? "Meal Updated Successfully":"Meal Successfully Created",
            visibilityTime: 4000, // time in milliseconds (5000ms = 5 seconds)
            autoHide: true,
          });

          await Delay(2000)
          router.navigate("/vendor/menu")
        }
              ...
              <View className='px-4 w-full mt-4'>
                <View style={styles.shadow_box} className='bg-white w-full rounded-lg p-4 flex flex-row items-center mt-4'>
                  ...
                  className={`w-24 h-24 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden ${thumbnail && 'border-2 border-custom-green'}`}>
                    ...
                    {id?
                      <Image 
                      source={{ uri: thumbnail? thumbnail : mealData?.thumbnail }} 
                ...
                <View
                  ...
                  <View>
                    <Text
                    style={{fontFamily: 'Inter-Medium'}}
                    className='text-[12px] text-custom-green ml-2 mt-1'
                    >
                      {(mealName !== "")  && 'Meal name'}
                    </Text>
                ...
                >
                  ...
                  >
                    {id? 'Update meal details':'Add meal to menu'}
                  </Text>
