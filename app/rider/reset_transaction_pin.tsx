...
export default function ResetTransactionPin(){
    ...
    const [currentPin, setCurrentPin] = useState(['', '', '', '']); // Store entered digits
    const [currentPinComplete, setCurrentPinComplete] = useState(false)
    ...
    const handleSubmit = async () => {
      try {
        if(!loading && currentPinComplete && newPinComplete && confirmPinComplete){
            if(newPin.join('') == confirmPin.join('')){
                if(currentPin.join('') !== newPin.join('')){
                    setLoading(true)
                    type DataResponse = { token: string; refresh: string; };
                    type ApiResponse = { status: string; message: string; data:DataResponse };
                    const res = await patchRequest<ApiResponse>(ENDPOINTS['account']['transaction-pin'], {
                        current_pin: currentPin.join(''),
                        new_pin: newPin.join(''), 
                    }, true);
                    ...
                    Toast.show({
                        type: 'success',
                        text1: "Transaction pin updated successfully",
                    ...
                    Toast.show({
                        type: 'error',
                        text1: "Your new pin cannot be your previous pin",
    ...
    const [currentFocusedIndex, setCurrentFocusedIndex] = useState<number | null>(null);
    ...
    const handleFocus = (index: number, which: string) => {
        switch (which) {
            case 'current':
                setCurrentFocusedIndex(index); // Update the focused index
                break;
            case 'new':
    ...
    const handleBlur = (which: string) => {
        switch (which) {
            case 'current':
                setCurrentFocusedIndex(null); // Update the focused index
                break;
            case 'new':
    ...
    const handleCurrentChangeText = (text: string, index: number) => {
        if (text.length === 1) {
            const newCode = [...currentPin];
            newCode[index] = text;
            setCurrentPin(newCode);

            // Focus on the next input if it's not the last one
            if (index < 3 && text) {
                currentInputRefs[index + 1].current?.focus();
            }
        } else if (text === '') {
        ...
        if(index == 3){
            setCurrentPinComplete(true)
        }else{
            setCurrentPinComplete(false)
        }
    };

    // Handle the key press for backspace navigation
    const handleCurrentKeyPress = (e: any, index: number) => {
        if (e.nativeEvent.key === 'Backspace' && currentPin[index] === '') {
        ...
        <SafeAreaView>
            <View className={`${theme == 'dark'? 'bg-gray-900' : ' bg-white'} w-full h-full flex items-center`}>
                ...
                <View className={`${theme == 'dark'? 'bg-gray-800' : ' bg-gray-100'} w-full`}>
                    <TitleTag withprevious={true} title='Reset Transaction Pin' withbell={false} />
                </View>
                ...
                <View className='w-full mt-8'>
                    ...
                    <Text
                    style={{fontFamily: 'Inter-Medium'}}
                    className={`${theme == 'dark'? 'text-gray-200' : ' text-gray-700'} text-center text-[13px] tracking-tighter mt-4`}
                    >
                        Current Pin
                    </Text>
                    <View className="flex-row justify-center mt-2 space-x-3">
                        {currentPin.map((digit, index) => (
                            <TextInput
                            key={index}
                            ref={currentInputRefs[index]}
                            style={{fontFamily: 'Inter-Regular'}}
                            className={`${theme == 'dark'? 'text-white' : ' text-gray-800'} w-12 h-12 text-center text-[16px] border-2 rounded-md  ${(currentFocusedIndex === index)? (' border-custom-green'):('border-gray-500')}`}
                            value={digit}
                            onFocus={() => handleFocus(index, 'current')} // Handle focus event
                            onBlur={()=>{handleBlur('current')}} // Handle blur event
                            onChangeText={(text) => handleCurrentChangeText(text, index)}
                            onKeyPress={(e) => handleCurrentKeyPress(e, index)}
                ...
                <TouchableOpacity
                    ...
                    className={`text-center mt-auto mb-10 ${(currentPinComplete && newPinComplete && confirmPinComplete )? 'bg-custom-green' : 'bg-custom-inactive-green'} ${loading && ('bg-custom-inactive-green')} relative rounded-xl p-4 w-[90%] self-center flex items-center justify-around`}
                    >
                        {loading && (
