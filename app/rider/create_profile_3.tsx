    ...
    const validateAccNumber = async (value: string) =>{
      ...
      const handleRequest = async () => {
        if(!loading && validateInput()){
            try {
                setLoading(true)
                const updatedProfile = await postRequest(ENDPOINTS['rider']['bank-details'], data, true);
                setLoading(false)
                Toast.show({
                    type: 'success',
